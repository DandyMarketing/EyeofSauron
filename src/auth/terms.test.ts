...
test('somebody who accepted the current version has accepted', () => {
  assert.equal(hasAcceptedCurrentTerms([{ terms_version: TERMS_VERSION }]), true);
});
...
test('an older acceptance alongside the current one still counts', () => {
  // Re-acceptance after a rewrite leaves two rows. The history is the point.
  assert.equal(
    hasAcceptedCurrentTerms([{ terms_version: '2020-01-01' }, { terms_version: TERMS_VERSION }]),
    true,
  );
});
