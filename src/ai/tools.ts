...
export const queryTools: Tool[] = [
  ...
  },
  {
    ...
    description:
      ...
      'READ lookback_coverage BEFORE QUOTING THE FIGURE. Deciding who is returning means looking for an earlier visit IN OUR DATA, so when the booking history does not reach back a full lookback the function finds nothing and counts that guest as NEW. It is a guest book started partway through: a month after you begin writing names down, almost everyone looks new. covered_pct says how much of the window exists, and anything under 100 means returning guests are understated and the new share overstated — by more, the earlier the period. The shortfall shrinks as the history fills, so a rise across the early months is the database filling up and not guests coming back more. Say so rather than reporting the trend. ' +
      'THE RATE IS A FLOOR, NOT A VERDICT ON HOSPITALITY. A guest who has left Singapore is indistinguishable from one who chose not to return, and these are CBD venues with heavy visitor and expatriate trade, so the share of RETURNABLE guests who come back is higher by an unknown amount. Never estimate that amount. Use the figure to COMPARE — venue against venue, month against month, where the same churn applies to both sides — and if it moves sharply ask whether the guest MIX changed before concluding regulars were lost. ' +
      'FOR "DOES THIS VENUE EARN A SECOND VISIT", USE query_guest_cohorts INSTEAD. This tool measures who was in the room and had been before, which falls when acquisition rises — good news drawn as bad. The cohort tool follows the people who first arrived together and gives each the same number of days to come back, which is the question about hospitality.',
    input_schema: {
      type: 'object' as const,
      properties: {
        venue_slug: {
  ...
  },
  {
    ...
    description:
      ...
      'IT CARRIES BOTH PERIODS FOR EVERY FIGURE, including the till block — average check, transactions and spend per head on the food-and-beverage basis. Build a two-period comparison table from this response alone. Do NOT reach for query_sales to fill the second column: that tool answers for one period per call, and a table assembled from one call of it prints an em-dash against data that is present. ' +
      'NOTE THE TWO SPEND-PER-HEAD FIGURES. The driver one is net sales ÷ covers; till.avg_spend_per_head is food + beverage ÷ covers, which is what query_sales reports and reads a few percent higher. Both are correct, they answer different questions, and mixing them within a table turns a definitional gap into an apparent change. The response labels which is which.',
    input_schema: {
      type: 'object' as const,
      properties: {
        venue_slug: {
  ...
  },
  {
    ...
    description:
      ...
      'IT IS THE MEASUREMENT THAT SEPARATES TWO DIFFERENT PROBLEMS. Booked covers falling can mean people stopped booking or that they started booking later, and those have different causes and different fixes. Covers alone cannot tell them apart; this can. ' +
      'QUOTE THE MEDIAN, NOT THE MEAN. Lead time has a long right tail — a Christmas party booked in September sits in the same column as a Tuesday booked that morning — so the mean follows one large event and reads as a change in behaviour that did not happen. Both are returned, and a mean far above the median is itself the finding: that month was carried by a few long-lead bookings. The caveats say when this applies. ' +
      'WALK-INS ARE EXCLUDED AND COUNTED. A walk-in is created as it arrives, so its lead time is zero by definition; including them would make this a measure of the walk-in ratio wearing the name of lead time. Never describe the booking counts here as total covers — use query_reservations for that. ' +
      'FUTURE DATES ARE EXCLUDED. A month still being booked shows only its early bookers, so it would report a lead time far longer than it finishes with, and the most recent months would always look best. ' +
      'READ THE BANDS, NOT ONLY THE MEDIAN. The same-day, 1-3, 4-7, 8-30 and 31+ shares say what actually moved: a median holding steady while the same-day share doubles is two populations changing in opposite directions, and the median alone hides it.',
    input_schema: {
      type: 'object' as const,
      properties: {
        venue_slug: {
  ...
  {
    name: 'create_composition_chart',
    description:
      'PART-TO-WHOLE: what something is MADE OF, and whether the mix is moving. Use this for "where do our guests come from", "which booking channels carry us", "how many are repeat guests", or any question whose answer is shares of a total. create_chart cannot draw these — it plots one number over time — so before this existed the answer was a paragraph of percentages, which is the hardest possible way to read percentages. ' +
      'TWO PICTURES OF THE SAME DATA, AND THE DEFAULT IS THE RIGHT ONE. chart_type "stacked_pct" draws one column per month, each a full 100%, so the mix can be compared across months — this is almost always what a briefing needs, because the question is nearly never "what is the mix" but "is the mix moving". chart_type "pie" draws a single donut for the whole range: it shows the mix and CANNOT show movement, so use it only when the reader wants the composition itself. Two pies side by side do not answer "has this changed" — people read angles badly across circles. ' +
      'THE COUNT IS ON BOTH CHARTS AND YOU MUST USE IT. A percentage rises when its numerator grows OR when its denominator shrinks, and those are opposite news: a venue that stops attracting new guests posts a RISING returning-guest share all the way down. The stacked chart prints each month\'s total above its column and the donut prints the total in the hole. When the response carries a share-trap caveat, say plainly that the total fell and quote the counts — never report a rising share as an improvement without checking it. ' +
      'METRICS. guest_source splits booked guests into new to the group, returning to this venue, and crossed from a sister venue — mutually exclusive and summing to booked guests. Walk-ins are excluded because SevenRooms gives each walk-in a fresh client id, so one would count as new on every visit. visit_mix splits VISITS into first, second, third and fourth-or-more — read as a funnel, and note the earliest months overstate first visits because a guest whose first visit predates the data looks new. booking_channels splits bookings by how they were made; a channel at zero is usually a broken integration rather than lost demand. ' +
      'guest_source AND visit_mix ARE DIFFERENT QUESTIONS. The first asks WHERE a returning guest returned to, which separates the venue\'s own job from the group\'s. The second asks HOW MANY TIMES anyone has come, which is the funnel. Do not present one as the other. ' +
      'Past six categories the smallest fold into "Other" so the shares still sum to a hundred, and a month too thin for its shares to mean anything has its total shown in amber. Omit venue_slug for the group as a whole, which is a real question but can hide two venues moving in opposite directions — the response says so when it applies.',
    input_schema: {
      type: 'object' as const,
      properties: {
        metric: {
          type: 'string',
          enum: ['guest_source', 'visit_mix', 'booking_channels'],
          description: 'What to break down. guest_source: new / returning here / crossed from sister. visit_mix: first / second / third / fourth-or-more visit. booking_channels: how the booking was made.',
        },
        start_date: { type: 'string', description: 'Start of range (inclusive), YYYY-MM-DD.' },
        end_date: { type: 'string', description: 'End of range (inclusive), YYYY-MM-DD.' },
        venue_slug: {
          type: 'string',
          description: 'One venue: "neon-pigeon", "fat-prince", or "super-firangi". Omit for the group as a whole.',
        },
        chart_type: {
          type: 'string',
          enum: ['stacked_pct', 'pie'],
          description: 'Omit for stacked_pct, which is the default and usually right: it shows whether the mix is MOVING. Use "pie" only when the reader wants the mix at a point in time and not its movement.',
        },
        title: { type: 'string', description: 'Optional title. Omit for a sensible default.' },
      },
      required: ['metric', 'start_date', 'end_date'],
